...
locals {
  ...
  # Datadog secret ARN: prefer infra-core remote state output; fall back to the
  # explicit override var (useful the first time infra-core has not been applied
  # yet and the output does not exist in state).
  datadog_api_key_secret_arn = try(
    data.terraform_remote_state.infra_core.outputs.datadog_api_key_secret_arn,
    var.datadog_api_key_secret_arn
  )

  # Whether Datadog is fully configured (non-empty ARN means the secret exists)
  datadog_enabled = local.datadog_api_key_secret_arn != ""
...
# count = 0 when datadog_api_key_secret_arn is empty (e.g. first-time bootstrap
# before infra-core has been applied with the Datadog module).
...
resource "aws_ecs_task_definition" "ms_video" {
  ...
  container_definitions = jsonencode(concat(
    ...
    # ── 2. Datadog Agent sidecar (only when Datadog is configured) ────────────
    ...
    [
      {
        ...
        # Application logs → Datadog via FireLens / Fluent Bit (when Datadog configured)
        # or → CloudWatch directly (when Datadog not yet configured).
        ...
        logConfiguration = local.datadog_enabled ? {
          ...
          options = {
            "awslogs-group"         = aws_cloudwatch_log_group.ms_video.name
            "awslogs-region"        = var.aws_region
            "awslogs-stream-prefix" = "ms-video"
          }
        }
    ...
    [
      {
        ...
        logConfiguration = local.datadog_enabled ? {
          ...
          options = {
            "awslogs-group"         = aws_cloudwatch_log_group.process_video.name
            "awslogs-region"        = var.aws_region
            "awslogs-stream-prefix" = "process-video"
          }
        }
